...
def scanMapSS(self, scan_list, src_img, controls, img_lib_path, conn):
    # Verify the world map window and ss again if locate map fails
    if not locateWorldMap(self, img_lib_path):
        src_img = self.capture_screen()
    matched_loc = []
    for item in scan_list:
        # src_img_gray = cv2.cvtColor(src_img, cv2.COLOR_BGR2GRAY)
        # template_img_gray = cv2.cvtColor(item['img'], cv2.COLOR_BGR2GRAY)
        template_match = cv2.matchTemplate(src_img, item['img'], cv2.TM_CCOEFF_NORMED)
        threshold = item['img_threshold']
        # Find the best match location
        while self.is_running:
            _, max_val, _, max_loc = cv2.minMaxLoc(template_match)
                ...
                if max_loc not in matched_loc:
                    ...
                    if old_cord_img[0] and new_cord_img[0]:
                        if not img.check_image_match(old_cord_img[1], new_cord_img[1]):
                            ...
                            # self.device_emu.shell('input keyevent KEYCODE_BACK')
                            # Undo the selection
                            k_tag_template = cv2.imread(img_lib_path + 'other/k_tag.png')
                            k_tag = img.get_template_coordinates(ss_cap, k_tag_template)
                            if k_tag[0]:
                                # print("Options Btn Found, ", options_btn)
                                self.invokeDeviceConsole("Updating the new location")
                                self.device_emu.shell(f'input tap ' + str(k_tag[1]) + ' ' + str(k_tag[2]))
                                sleep(0.5)
                            return False
                    ...
                    check_query = ""
                    query_values = {}
                    add_query_values = {"type": item['type'], "b_id": None, "m_id": None, "s_id": None,
                                        "show_scan": False, "expires_at": datetime.now() + timedelta(hours=8)}
                    # Type Boss Scan
                    if item['type'] == 'boss_scan':
                        share_info = bossScan(self, item, controls, center_x, center_y, scan_list, img_lib_path)
                        # print(share_info) #(True, ({'x': '393', 'y': '325'}, (True, 120, 247), 33, 192))
                        check_query = check_boss_collective_item_query
                        if share_info[0]:
                            query_values = {'cords': json.dumps(share_info[1][0]), 'b_id': item['level_id']}
                            add_query_values["b_id"] = item['level_id']
                            # print(query_values)
                    # Type Normal monster Scan
                    elif item['type'] == 'monster_scan':
                        share_info = monsterScan(self, item, controls, center_x, center_y, scan_list,
                                                 img_lib_path)
                        check_query = check_monster_collective_item_query
                        if share_info[0]:
                            query_values = {'cords': json.dumps(share_info[1][0]), 'm_id': item['id']}
                            add_query_values["m_id"] = item['id']
                    # Type Scoutables Scan
                        ...
                        share_info = scoutablesScan(self, item, controls, center_x, center_y, scan_list,
                                                    img_lib_path)
                        check_query = check_scoutables_collective_item_query
                        if share_info[0]:
                            query_values = {'cords': json.dumps(share_info[1][0]), 's_id': item['id']}
                            add_query_values["s_id"] = item['id']
                    # Type Tile Scan
                    elif item['type'] == 'tile_scan':
                        share_info = tileScan(self, item, controls, center_x, center_y, scan_list, img_lib_path)

                    if share_info[0] is not True:
                        # self.device_emu.shell('input keyevent KEYCODE_BACK')
                        return False
                    if dbUtils.select_query(conn, check_query, query_values).fetchone() is None:
                        # print("item doesnt exist")
                        conn.execute("PRAGMA foreign_keys = ON")
                        add_query_values["cords"] = json.dumps(share_info[1][0])

                        if controls['share_collective']:
                            add_query_values["show_scan"] = True
                            self.device_emu.shell('input keyevent KEYCODE_BACK')
                        else:
                            add_query_values["show_scan"] = False
                        dbUtils.execute_query(conn, add_collective_query, add_query_values)
                        conn.commit()
                    else:
                        # print("item already exist")
                        self.invokeDeviceConsole("Skipping already scanned items")
                        self.device_emu.shell('input keyevent KEYCODE_BACK')
                        return False

                    # Share based on the option selected
                    confirm_share_img = cv2.imread(img_lib_path + 'other/confirm_share.png')
                    if controls['share_whisper'] and share_info[0]:
...
def getShareWindowDetails(share_window, img_lib_path):
    ...
    xy_cords = {'x': int(xy_cords[0]), 'y': int(xy_cords[1])}
...
def monsterScan(self, item, controls, center_x, center_y, scan_list, img_lib_path):
    ...
    if attack_btn[0] is False:
        ...
        time.sleep(1)
        self.device_emu.shell(
            f'input tap ' + str(center_x + item['click_pos']['x']) + ' ' + str(center_y + item['click_pos']['y']))
        return False, None
