    ...
    def changeProfile(self):
        ...
        try:
            ...
            for key in profile_control:
                ...
                try:
                    widget = getattr(self.ui, f"{key}{index}")
                except Exception as e:
                    if f"{key}{index}" in self.dynamic_ui:
                        widget = self.dynamic_ui[f"{key}{index}"]
                    else:
                        widget = self.findChild(eval(value['type']), f"{key}{index}")
