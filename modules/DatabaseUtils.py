...
def getAllScoutablesData():
    ...
    for scout in scoutables_table:
        tmp_dict = {'id': scout[0], 'name': scout[1], 'type': scout[2], 'system': scout[3], 'preview_image': scout[4],
                    'img_540p': scout[5], 'img_1080p': scout[6], 'img_id': scout[7]}
...
def getAllBossMonsterData():
    ...
    for monster in boss_monster_table:
        tmp_dict = {'id': monster[0], 'preview_name': monster[1], 'monster_title': monster[2],
                    ...
                    'monster_logic_id': monster[6], 'level': monster[7], 'name': monster[8], 'size': monster[9],
                    'preview_image': monster[10], 'img_540p': monster[11], 'img_1080p': monster[12],
                    'img_threshold': monster[13], 'click_pos': json.loads(monster[14]), 'img_id': monster[15]}
        ...
        if monster_list1["id"] not in seen_keys:
            for monster_list2 in tmp_boss_monster_list:
                if monster_list1["id"] == monster_list2["id"]:
                    levels.append(
                        {'level': monster_list2["level"], 'name': monster_list2["name"], 'size': monster_list2["size"]})
            ...
            tmp_dict = {'id': monster_list1["id"], 'preview_name': monster_list1["preview_name"],
                        ...
                        'img_540p': monster_list1["img_540p"],
                        'img_1080p': monster_list1["img_1080p"], 'img_threshold': monster_list1["img_threshold"],
