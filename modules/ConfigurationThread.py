...
class ConfigurationThread(QThread):
    ...
    def capture_screen(self, kick_timer=True, ads=True, game_settings=True):
        ...
        if kick_timer and img.check_template_match(src_img, restart_img):
            ...
            self.invokeDeviceConsole(f"Kick timer activated for {self.emulator_control['game_settings']['kick_timer']} mins")
        ...
        if game_settings:
            ...
            if self.emulator_control['game_settings']['add_break']['add_break']:
                ...
                if break_time['from'] <= current_time <= break_time['to']:
                    ...
                    # TODO make sure it bypassed game update window on the loading screen
        return src_img
    ...
    def runWorldMapScan(self):
        ...
        while self.is_running:
            ...
            for times in range(random_number):
                ...
                check_counter = 0
                scan_map = False
                while check_counter < 3 and scan_map is False:
                    src_img = self.capture_screen()
                    scan_map = wm_scan.scanMapSS(self, scan_list, src_img, controls, img_lib_path, conn)
                    check_counter += 1
