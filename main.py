...
class MainWindow(QMainWindow):
        ...
        for emulator_index in range(self.total_emulator):
            ...
            for item in items:
                if item.objectName().startswith("item_"):
                    ...
                    self.dynamic_ui[object_name] = toggle
    ...
    def loadScoutablesControlsScanMap(self):
        ...
        for emulator_index in range(self.total_emulator):
            ...
            for scoutable in all_scoutables:
                # print(scoutable)
                object_name = generateObjectName(scoutable["name"]) + str(emulator_index + 1)
                cbox = QCheckBox(scoutable["name"])
                cbox.setObjectName(object_name)
                self.dynamic_ui[object_name] = cbox
                scoutables_layout.addWidget(cbox)
            # self.findChild(QFrame, "scoutables_scan_frame_" + str(emulator_index + 1)).setLayout(scoutables_layout)
            getattr(self.ui, f"scoutables_scan_frame_{(emulator_index + 1)}").setLayout(scoutables_layout)
    ...
    def loadNormalMonsterControlsScanMap(self):
        ...
        for emulator_index in range(self.total_emulator):
            ...
            for monster in all_normal_monsters:
                # print(monster)
                object_name = generateObjectName(monster["name"]) + str(emulator_index + 1)
                cbox = QCheckBox(monster["name"])
                cbox.setObjectName(object_name)
                self.dynamic_ui[object_name] = cbox
                normal_monster_layout.addWidget(cbox)
            # self.findChild(QFrame, "normal_monster_scan_frame_" + str(emulator_index + 1)).setLayout(normal_monster_layout)
            getattr(self.ui, f"normal_monster_scan_frame_{(emulator_index + 1)}").setLayout(normal_monster_layout)
        ...
        for emulator_index in range(self.total_emulator):
            ...
            for monster in boss_monsters:
                ...
                if monster['enable_map_scan'] == 1:
                    ...
                    object_name = generateObjectName(monster["preview_name"]) + "sm_" + str(emulator_index + 1)
                    cbox.setObjectName(object_name)
            ...
            for monster in event_monsters:
                if monster["monster_logic_id"] == 2 and monster['enable_map_scan'] == 1:
                    ...
                    self.dynamic_ui[checkbox_obj_name] = check_box
                    self.dynamic_ui[combobox_obj_name] = combo_box
    ...
    def loadMonsterControlsJoinRally(self):
        ...
        for emulator_index in range(self.total_emulator):
            ...
            for monster in boss_monsters:
                # print(monster)
                cbox = QCheckBox(monster["preview_name"])
                object_name = generateObjectName(monster["preview_name"]) + "jr_" + str(emulator_index + 1)
                cbox.setObjectName(object_name)
                boss_monster_layout.addWidget(cbox)
                self.dynamic_ui[object_name] = cbox
            for monster in event_monsters:
    ...
    def setUpMoreActivitiesFrames(self):
        ...
        for emulator_index in range(self.total_emulator):
            for frame in frames:
                ...
                getattr(self.ui, object_name).setStyleSheet(css)
    ...
    def monster_checkbox_change_logic3(self):
        ...
        checkbox = self.dynamic_ui[self.sender().objectName()]
        # combobox = self.findChild(QComboBox, self.sender().objectName().replace("checkbox_", "combobox_"))
        combobox = self.dynamic_ui[self.sender().objectName().replace("checkbox_", "combobox_")]
    ...
    def emulatorNameChange(self):
        ...
        if edit_name == f"emu_name_{str(index)}":
            if edit.text() == "":
                ...
                getattr(self.ui, f"btn_emu_{index}").setText(f"Emulator {index}")
                # self.findChild(QGroupBox, "groupBox_console_" + str(index)).setTitle("Emulator " + str(index) + " Console")
                getattr(self.ui, f"groupBox_console_{index}").setTitle(f"Emulator {index} Console")
            else:
                # self.findChild(QPushButton, "btn_emu_" + str(index)).setText(edit.text())
                getattr(self.ui, f"btn_emu_{index}").setText(edit.text())
                # self.findChild(QGroupBox, "groupBox_console_" + str(index)).setTitle(edit.text() + " Console")
                getattr(self.ui, f"groupBox_console_{index}").setTitle(edit.text() + " Console")
            ...
            if checkbox.isChecked():
                ...
                for monster in all_boss_monsters:
                    if monster['preview_name'] == checkbox.text():
                        ...
                        monsters['img_threshold'] = monster['img_threshold']
                        monsters['click_pos'] = monster['click_pos']
                        monsters['level_id'] = monster['levels'][0]['level_id']
                        ...
                        for combobox_lv in combo_box.checkedIndices():
                            ...
                            for level in monster['levels']:
                                if level['level'] == combobox_lv - 1:
                                    monsters['level_id'] = level['level_id']
